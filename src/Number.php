...
namespace nguyenanhung\Libraries\Number;
...
if (!class_exists('nguyenanhung\Libraries\Number\Number')) {
    /**
    ...
    class Number
    {
        ...
        /**
         * @var string the regex for a comma-separated number (e.g., "1,000")
         */
                    ...
                    if (is_numeric($b)) {
                        ...
                        if (is_numeric($epsilon)) {
                            ...
                            if ($epsilon > 0) {
                                ...
                                throw new InvalidArgumentException(__METHOD__ . " expects the third parameter, epsilon, to be greater than zero");
                            }
                        } else {
                            throw new InvalidArgumentException(__METHOD__ . " expects the third parameter, epsilon, to be a number");
                        }
                    } else {
                        throw new InvalidArgumentException(__METHOD__ . " expects the second parameter, b, to be a number");
                    }
                } else {
                    throw new InvalidArgumentException(__METHOD__ . " expects the first parameter, a, to be a number");
                }
            } else {
                throw new BadMethodCallException(__METHOD__ . " expects two or three numeric arguments");
            }
                    ...
                    if (!$hasLower || is_numeric($lower)) {
                        ...
                        if (!$hasUpper || is_numeric($upper)) {
                            ...
                            if (!$hasLower || !$hasUpper || $upper >= $lower) {
                                ...
                                throw new InvalidArgumentException(__METHOD__ . " expects the third parameter, the upper bound ({$upper}), to be " . "greater than or equal to the second parameter, the lower bound ({$lower})");
                            }
                        } else {
                            throw new InvalidArgumentException(__METHOD__ . " expects the third parameter, the upper bound, to be a number");
                        }
                    } else {
                        throw new InvalidArgumentException(__METHOD__ . " expects the second parameter, the lower bound, to be a number");
                    }
                } else {
                    throw new InvalidArgumentException(__METHOD__ . " expects the first parameter, number, to be a number");
                }
            } else {
                throw new BadMethodCallException(__METHOD__ . " expects two or three numeric parameters");
            }
                ...
                if (is_numeric($number)) {
                    ...
                    if (is_numeric($multiple)) {
                        ...
                        if ($multiple > 0) {
                            ...
                            throw new InvalidArgumentException(__METHOD__ . " expects parameter two, the multiple, to be greater than zero");
                        }
                    } else {
                        throw new InvalidArgumentException(__METHOD__ . " expects parameter two, the multiple, to be a number");
                    }
                } else {
                    throw new InvalidArgumentException(__METHOD__ . " expects parameter one, the number, to be a number");
                }
            } else {
                throw new BadMethodCallException(__METHOD__ . " expects one or two numeric parameters");
            }
                ...
                if (is_numeric($number)) {
                    ...
                    if (is_numeric($multiple)) {
                        ...
                        if ($multiple > 0) {
                            ...
                            throw new InvalidArgumentException(__METHOD__ . " expects the second parameter, the multiple, to be greater than zero");
                        }
                    } else {
                        throw new InvalidArgumentException(__METHOD__ . " expects the second parameter, the multiple, to be numeric");
                    }
                } else {
                    throw new InvalidArgumentException(__METHOD__ . " expects the first parameter, the number, to be numeric");
                }
            } else {
                throw new BadMethodCallException(__METHOD__ . " expects one or two numeric parameters");
            }
                ...
                if (is_string($datatype)) {
                    ...
                    if (is_numeric($number) && self::isInt($number) && $number > 0) {
                        ...
                        switch (strtolower($datatype)) {
                            ...
                            default:
                                throw new InvalidArgumentException(__METHOD__ . " expects parameter two to be a valid datatype name such as: " . "'tiny[int]', 'small[int]', 'medium[int]', 'int[eger]', or 'big[int]'," . "{$datatype} given");
                        }
                    }
                } else {
                    throw new InvalidArgumentException(__METHOD__ . " expects parameter two to be a string, the datatype name");
                }
            } else {
                throw new BadMethodCallException(__METHOD__ . "() expects two parameters, a number and a datatype name");
            }
        ...
        {
            return is_numeric($number) || (is_string($number) && preg_match(self::REGEX_NUMBER_MIXED, $number)) || (is_string($number) && preg_match(self::REGEX_NUMBER_COMMA, $number));
        }
        ...
        {
            return $number === 0 || $number === 0.0 || $number === '0' || $number === '0.0';
        }
                ...
                if (is_numeric($number)) {
                    ...
                    if (is_numeric($max)) {
                        ...
                        if ($max > 0) {
                            ...
                            throw new InvalidArgumentException(__METHOD__ . " expects parameter two, the max, to be greater than zero");
                        }
                    } else {
                        throw new InvalidArgumentException(__METHOD__ . " expects parameter two, the max, to be a number");
                    }
                } else {
                    throw new InvalidArgumentException(__METHOD__ . " expects parameter one, the number, to be a number");
                }
            } else {
                throw new BadMethodCallException(__METHOD__ . " expects two numeric parameters");
            }
        ...
        public static function bytes($size)
        {
            ...
            $unit = ArrayHelper::get($matches, 2);
            if (empty($unit)) {
                $unit = 'B';
            }
